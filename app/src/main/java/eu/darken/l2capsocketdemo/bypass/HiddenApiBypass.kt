package eu.darken.l2capsocketdemo.bypass
 ...
 *
 * NOTE for the Google issue tracker reviewer: this exists ONLY to demonstrate that the
 * platform's transport already supports BR/EDR L2CAP on PSM 0x1001. It is fragile, breaks
 * with each ART internals change, and is exactly the pattern Google's non-SDK-interface
 * guidance asks third-party apps to file public-API requests for instead.
 */
        ...
        try {
            for (i in 0 until numMethods) {
                ...
                if (name == "setHiddenApiExemptions" && params.size == 1 && params[0] == Array<String>::class.java) {
                    ...
                    exemptionsSet = true
                    Log.v(TAG, "setHiddenApiExemptions OK: ${allPrefixes.contentToString()}")
                }
